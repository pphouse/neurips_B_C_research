...
(iii)~generalizes to a held-out protein domain (train BRCT, test RING). Probing the codes,
the shared representation predicts loss-of-function (AUROC \lofShared{}) and protein domain,
while modality-private codes capture complementary structure; the two models' functional
axes map to nearly the same shared latent (cosine \sharedCos{}). Finally, we test whether
this representational alignment yields \emph{causal} cross-model handles: injecting the
shared functional direction shifts ESM-2's variant score in a DMS-correlated way, but the
same intervention on Evo\,2's likelihood is non-specific---representational alignment does
not imply interchangeable causal control. We release code, activations, and the full harness.
\end{abstract}
...
\textbf{(iv)~Alignment $\neq$ causal exchange.} Representational alignment (cosine \sharedCos{})
