...
def main():
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            ...
            ev_tr = tr if len(tr)<=200 else np.random.RandomState(0).choice(tr,200,replace=False)
            ev_te = te if len(te)<=200 else np.random.RandomState(0).choice(te,200,replace=False)
            str_ = scores(ev_tr); ste = scores(ev_te)
        au_tr = roc_auc_score(y[ev_tr], str_) if len(np.unique(y[ev_tr])) > 1 else float("nan")
        au_te = roc_auc_score(y[ev_te], ste) if len(np.unique(y[ev_te])) > 1 else float("nan")
